...
contract HouseVault is ERC20, Ownable, Pausable, ReentrancyGuard {
    ...
    event RehabClaimed(address indexed user, uint256 stake, uint256 shares, uint256 duration, uint256 creditIssued);
    ...
    constructor(IERC20 _asset, uint256 _corrAsymptoteBps, uint256 _corrHalfSatPpm, uint256 _maxLegsPerGroup)
        ERC20("ParlayVoo", "VOO")
        Ownable(msg.sender)
    {
        ...
        corrConfig = CorrelationConfig({
            corrAsymptoteBps: _corrAsymptoteBps, corrHalfSatPpm: _corrHalfSatPpm, maxLegsPerGroup: _maxLegsPerGroup
        });
    }
    ...
    function routeFees(uint256 feeToLockers, uint256 feeToSafety, uint256 feeToVault) external onlyEngine nonReentrant {
