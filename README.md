...
```
parlaycity/
├── packages/nextjs/                    # Next.js 14 frontend (Vercel-deployed)
│   └── src/
...
`packages/nextjs/.env.local` is auto-generated by `make deploy-local` via `scripts/sync-env.sh`. Only `NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID` needs manual setup.
