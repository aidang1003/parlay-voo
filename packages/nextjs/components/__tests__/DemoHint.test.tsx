...
describe("DemoHint", () => {
  it("renders when visible is true", () => {
    render(<DemoHint sessionKey="demo:test" step={1} message="Test hint message" visible={true} />);
  ...
  it("does not render when visible is false", () => {
    render(<DemoHint sessionKey="demo:test" step={2} message="Hidden hint" visible={false} />);
    expect(screen.queryByTestId("demo-hint-2")).not.toBeInTheDocument();
  });

  it("dismiss hides the hint and persists to sessionStorage", () => {
    render(<DemoHint sessionKey="demo:test" step={1} message="Dismissable hint" visible={true} />);
    expect(screen.getByTestId("demo-hint-1")).toBeInTheDocument();
  ...
  it("stays hidden when sessionStorage has dismissed=true", () => {
    ...
    render(<DemoHint sessionKey="demo:test" step={1} message="Already dismissed" visible={true} />);
    expect(screen.queryByTestId("demo-hint-1")).not.toBeInTheDocument();
  });

  it("renders correct step number", () => {
    render(<DemoHint sessionKey="demo:step3" step={3} message="Step 3 hint" visible={true} />);
