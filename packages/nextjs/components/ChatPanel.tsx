...
export function ChatPanel() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex h-[32rem] w-96 flex-col overflow-hidden rounded-2xl border border-white/10 bg-surface shadow-2xl shadow-brand-purple/20">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
        {messages.length === 0 && (
          <div className="space-y-3">
            <p className="text-xs text-gray-400">Ask me about markets, quotes, risk, or protocol health.</p>
          ...
          <div key={m.id} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
            <div
              className={`max-w-[85%] rounded-xl px-3 py-2 text-sm ${
                m.role === "user" ? "bg-brand-purple text-white" : "bg-white/5 text-gray-200"
              }`}
            >
                ...
                if (ti.state === "result") {
                  return (
                    <details key={i} className="mt-2 text-xs">
                      <summary className="cursor-pointer text-brand-purple-1">Tool: {ti.toolName}</summary>
      ...
      {/* Input */}
      <form onSubmit={handleSubmit} className="border-t border-white/10 bg-surface-2 px-3 py-3">
