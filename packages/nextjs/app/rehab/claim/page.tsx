    ...
    <div className="mx-auto max-w-2xl space-y-8 pb-12">
      <section>
        ...
        <p className="mt-2 text-gray-400">
          When a parlay loses, your stake goes into the pool and accrues to a per-wallet balance. Lock that balance up
          and the vault mints you bet-only credit worth one year of yield on the principal — regardless of how long you
          lock for.
        </p>
      </section>
      ...
      <section className="rounded-2xl border border-amber-500/30 bg-gradient-to-br from-amber-950/40 to-yellow-950/20 p-6">
        <p className="text-[10px] font-medium uppercase tracking-wider text-amber-400/70">Unclaimed losses</p>
        <p className="mt-1 text-4xl font-black tabular-nums text-amber-200">${fmtUsdc(claimable)}</p>
      ...
      <section className="space-y-4 rounded-2xl border border-white/5 bg-gray-900/50 p-6">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-400">
            Longer locks don&apos;t earn more credit — they demonstrate conviction and keep your principal deeper in the
            pool. Minimum lock is{" "}
        ...
        <div className="flex items-center gap-3 pt-2">
          <label className="text-xs font-medium uppercase tracking-wider text-gray-500">Custom (years)</label>
          <input
            type="number"
            min={minYears}
            max={50}
      ...
      <section className="grid gap-4 rounded-2xl border border-white/5 bg-gray-900/30 p-6 sm:grid-cols-2">
        <div>
          <p className="text-[10px] font-medium uppercase tracking-wider text-gray-500">You lock</p>
          <p className="mt-1 text-2xl font-bold tabular-nums text-white">${fmtUsdc(claimable)}</p>
        ...
        <div className="sm:text-right">
          ...
          <p className="mt-1 text-2xl font-bold tabular-nums text-amber-200">${fmtUsdc(creditPreview)}</p>
      ...
      <section className="space-y-3 rounded-2xl border border-white/5 bg-gray-950/60 p-6 text-sm text-gray-400">
        ...
        <ul className="space-y-2 pl-1">
          <li>
            <span className="font-semibold text-white">Your losses stay in the pool.</span> They back fresh VOO shares
            which are locked on your behalf — you don&apos;t withdraw USDC here.
          </li>
          <li>
            <span className="font-semibold text-white">Credit is bet-only.</span> Use it on lossless parlays. Wins pay
            out as PARTIAL-tier locks; losses just burn the credit — your locked principal is untouched.
          </li>
          <li>
            <span className="font-semibold text-white">Credit size is fixed.</span> Always {aprPct ?? 6}% of your
            claimed amount, regardless of lockup length. Longer locks only affect when your principal becomes spendable
            again.
          </li>
          <li>
            <span className="font-semibold text-white">No early exit.</span> Rehab locks can&apos;t be unwound before
            expiry. When they do expire, the principal returns to LPs — credit you&apos;ve already earned is yours to
            keep.
          </li>
        </ul>
      </section>
