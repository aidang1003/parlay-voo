...
import { LegBuildError, type LegInput, buildLegs } from "~~/lib/quote/build-legs";
import {
  ANVIL_ACCOUNT_0_KEY,
...
export async function POST(req: Request) {
  ...
  const chainId = Number(process.env.NEXT_PUBLIC_CHAIN_ID || String(LOCAL_CHAIN_ID)) as SupportedChainId;
  ...
  const useUma = chainId === BASE_MAINNET_CHAIN_ID || (chainId === BASE_SEPOLIA_CHAIN_ID && oracleMode === "uma");
  const oracleAddr = useUma ? chainContracts.UmaOracleAdapter?.address : chainContracts.AdminOracleAdapter?.address;

  if (!signerKey) {
    return NextResponse.json(
      { error: "neither HOT_SIGNER_PRIVATE_KEY nor WARM_DEPLOYER_PRIVATE_KEY is set" },
      { status: 500 },
    );
  }
