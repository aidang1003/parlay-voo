...
- `script/DeployOnboardingFaucet.s.sol` -- standalone faucet deploy (excluded from main flow; broadcasts as `QUOTE_SIGNER_PRIVATE_KEY`)
- `src/peripheral/OnboardingFaucet.sol` -- testnet ETH (one-shot) + USDC (24h cooldown) drip; not part of `Deploy.s.sol`

**Frontend (`packages/nextjs/src/`):**
- `app/page.tsx` -- onboarding landing (5-step checklist for brand-new crypto users)
- `app/parlay/page.tsx` -- parlay builder (main app surface; was `/`)
- `app/_onboard/` -- per-step onboarding components + EnterAppCTA
- `middleware.ts` -- redirects first-time visitors (no `onboarding_completed` cookie) to `/`
- `lib/onboarding.ts` -- localStorage + cookie helpers (`getCompleted`, `setCompleted`, `resetOnboarding`)
...
- `lib/hooks/index.ts` -- barrel for domain hooks (leg, usdc, vault, lock, parlay, ticket, onboarding)
- `lib/hooks/onboarding.ts` -- `useOnboardingProgress` (5-step flags), `useOnboardingFaucet` (claim ETH/USDC)
