...
export default function AdminDebugPage() {
  ...
  if (!isTestnet) {
    return <div className="py-16 text-center text-gray-500">Disabled on this chain.</div>;
  }
    ...
    <div className="space-y-10">
      <header>
        ...
        <p className="mt-2 text-gray-400">
          Testnet-only tooling. Mint MockUSDC, poke the Polymarket DB, resolve legs manually.
        </p>
      </header>
...
// ── Developer tools (SE2 built-ins) ───────────────────────────────────────

function DeveloperToolsSection() {
  return (
    <Section title="Developer Tools">
      <p className="mb-4 text-xs text-gray-500">
        Built-in Scaffold-ETH 2 utilities. Use the contract debugger to call functions directly via auto-generated UI,
        or the local block explorer to inspect blocks + recent transactions on the running anvil chain.
      </p>
      <div className="grid gap-4 md:grid-cols-2">
        <ToolLink
          href="/debug"
          label="Contract Debugger"
          description="Auto-generated UI for every contract in deployedContracts.ts."
          icon={<Wrench className="h-4 w-4" />}
        />
        <ToolLink
          href="/blockexplorer"
          label="Block Explorer"
          description="Local-chain blocks + transactions. Click a tx hash for status, gas, and revert reason."
...
function ToolLink({
  ...
  return (
    <Link
      href={href}
      className="group rounded-lg border border-white/5 bg-gray-900/40 p-4 transition-colors hover:border-brand-pink/40 hover:bg-gray-900/60"
    >
      <div className="flex items-center gap-2">
        <span className="grid h-7 w-7 place-items-center rounded-md border border-white/10 bg-white/5 text-gray-300 transition-colors group-hover:border-brand-pink/40 group-hover:text-brand-pink">
          {icon}
        </span>
        <span className="font-semibold text-gray-200 group-hover:text-white">{label}</span>
      </div>
...
function MintSection() {
  ...
  return (
    <Section title="Mint MockUSDC">
      ...
      <p className="mt-1 text-right text-xs text-gray-500">= ${amount.toLocaleString("en-US")} USDC</p>
...
function DatabaseSection() {
  return (
    <Section title="Database">
      <p className="mb-4 text-xs text-gray-500">
        Proxies attach CRON_SECRET server-side so Vercel-hosted testnets can still trigger these without the token
  ...
  return (
    ...
    >
      <p className="mb-4 text-xs text-gray-500">
        ...
        <code className="text-gray-300">AdminOracleAdapter.resolve()</code> server-side with{" "}
        <code className="text-gray-300">WARM_DEPLOYER_PRIVATE_KEY</code>. YES pays YES bettors, NO pays NO bettors, VOID
        refunds the leg.
      </p>
            ...
            <thead className="border-b border-white/5 bg-white/5 text-left text-xs uppercase tracking-wider text-gray-500">
              <tr>
                ...
                <th
                  ...
                  title="Active tickets that bet YES vs NO on this leg, with summed stake."
                >
                  Positions
                </th>
                ...
                return (
                  <tr key={key} className="align-top text-gray-300">
                    ...
                    <td className="px-4 py-3 max-w-[320px]">
                      ...
                      {state?.error && <div className="mt-1 text-[11px] text-neon-red line-clamp-2">{state.error}</div>}
                      {state?.result && <div className="mt-1 text-[11px] text-emerald-400">{state.result}</div>}
                    </td>
                    ...
                    <td className="px-4 py-3 text-right font-mono text-xs leading-tight">
                      <div className="text-emerald-300">
                        {leg.yesCount} <span className="text-gray-500">·</span> ${fmtStake(leg.yesStake)}
                      </div>
                      <div className="text-red-300">
                        {leg.noCount} <span className="text-gray-500">·</span> ${fmtStake(leg.noStake)}
                      </div>
                    </td>
...
function RecentResolutionsSection() {
  ...
  return (
    ...
    >
      <p className="mb-4 text-xs text-gray-500">
        Audit trail of the last 20 leg resolutions, newest first. Once resolved on-chain, a leg is immutable — this list
        is read-only.
      </p>
      ...
      {!isLoading && rows.length === 0 && <div className="py-6 text-center text-gray-500">No resolved legs yet.</div>}
