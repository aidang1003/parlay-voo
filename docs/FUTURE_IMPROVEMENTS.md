...
**Current state:** Users lock VOO shares for 30/60/90 days and receive a tier multiplier (1.1x/1.25x/1.5x) on fee distributions. Early withdrawal incurs a linear penalty on the locked principal. The locked capital is idle — it doesn't increase the vault's betting capacity or generate yield.
...
2. **Locked capital doesn't improve vault capacity.** Locked VOO sits in the LockVault contract and doesn't count toward HouseVault's `totalAssets()` or `freeLiquidity()`. Locking actually *reduces* the vault's ability to underwrite bets, hurting both LPs and bettors.
...
- **Locked shares should remain productive.** Instead of transferring VOO out of HouseVault, the LockVault could track lock commitments as a lien on shares — the capital stays in HouseVault contributing to TVL and betting capacity, while the lock prevents the user from withdrawing.
...
```
LockVault v2:
- lock() creates a commitment record (no VOO transfer)
