 ...
 * AdminOracleAdapter.resolve() server-side with WARM_DEPLOYER_PRIVATE_KEY so no
 * external binaries (forge / pnpm) are needed on Vercel.
 *
 * Body: { legId: string, status: 1 | 2 | 3 } (1=Won/YES, 2=Lost/NO, 3=Voided).
 */
import { NextResponse } from "next/server";
import { type Chain, type Hex, createPublicClient, createWalletClient, http, parseAbi } from "viem";
