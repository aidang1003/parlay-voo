...
import type { PolymarketOrderBook } from "~~/utils/parlay";

const book = (bids: Array<[number, number]>, asks: Array<[number, number]>): PolymarketOrderBook => ({
  tokenId: "tok",
...
describe("bookMidPpm", () => {
  ...
  it("returns null for a crossed/inverted book", () => {
    ...
    expect(bookMidPpm(book([[0.8, 10]], [[0.7, 10]]))).toBeNull();
  });
