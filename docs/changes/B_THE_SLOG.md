...
- U-5: cart row in `ParlayBuilder.tsx` now shows per-leg multiplier (`effectiveOdds`) next to the question — uses the existing helper (no inline division)
- U-6: `MultiplierClimb.tsx` swaps the polyline path for a Catmull-Rom-to-Bezier `smoothPath` so the rocket flight curves; the dash-reveal animation now scales straight-line distance to the actual curved path length so segment reveals stay aligned with leg resolution