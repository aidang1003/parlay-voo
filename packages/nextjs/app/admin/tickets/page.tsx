...
import { formatUSDC, mapStatus } from "~~/lib/utils";
    ...
    <div className="space-y-6">
      ...
      {runResult && (
        <div className="rounded-lg border border-emerald-500/20 bg-emerald-500/5 px-4 py-3 text-sm text-emerald-300">
          <span className="font-semibold">Settlement run complete.</span> Resolved {runResult.resolved} · Settled{" "}
          {runResult.settled} · Skipped {runResult.skipped}
        ...
        {TABS.map(t => (
          <button
            ...
            className={`rounded-lg px-3 py-2 text-sm font-semibold transition-all ${
              tab === t.key ? "gradient-bg text-white shadow-lg" : "text-gray-500 hover:text-gray-300"
            }`}
          >
            {t.label}
            {tabCounts[t.key] > 0 && (
              ...
              {filtered.map(r => (
                <tr key={r.id.toString()} className="text-gray-300 hover:bg-white/5">
                  ...
                  <td className="px-4 py-3 text-right">
                    <Link href={`/ticket/${r.id.toString()}`} className="text-xs text-brand-pink hover:underline">
                      View →
                    </Link>
                  </td>
                </tr>
      ...
      {!isLoading && rows.length > 0 && filtered.length === 0 && (
        <div className="py-16 text-center text-gray-500">No tickets with status &ldquo;{tab}&rdquo;.</div>
      )}
    </div>
  );
}
