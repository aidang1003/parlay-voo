...
import { assessRisk, getLegStatus, getProtocolConfig, getQuote, getVaultHealth, listMarkets } from "~~/lib/mcp/tools";

const TOOL_DEFINITIONS = [
  {
    name: "list_markets",
    description: "List available ParlayVoo betting markets. Optionally filter by category.",
    inputSchema: {
      type: "object",
      properties: {
        category: {
          type: "string",
          description: "Filter by category: crypto, defi, nft, policy, economics, trivia, ethdenver, nba",
        },
      },
    },
  },
  {
    name: "get_quote",
    description: "Get a parlay quote -- multiplier, potential payout, and fees for given legs and stake.",
    inputSchema: {
      type: "object",
      properties: {
        legIds: {
  ...
  },
  {
    ...
    description: "Check HouseVault health: total assets, reserved, free liquidity, utilization.",
  ...
  {
    name: "assess_risk",
    description: "Assess parlay risk using Kelly criterion. Returns BUY/REDUCE_STAKE/AVOID recommendation.",
    inputSchema: {
      type: "object",
      properties: {
        legIds: {
  ...
  },
  {
    ...
    description: "Get protocol configuration: fees, limits, contract addresses, chain info.",
    inputSchema: { type: "object", properties: {} },
  },
];
...
const TOOL_EXECUTORS: Record<string, (args: Record<string, unknown>) => Promise<unknown>> = {
  ...
  get_quote: args => getQuote(args as { legIds: number[]; stake: number }),
  get_vault_health: () => getVaultHealth(),
  get_leg_status: args => getLegStatus(args as { legId: number }),
  assess_risk: args => assessRisk(args as { legIds: number[]; stake: number; bankroll?: number }),
