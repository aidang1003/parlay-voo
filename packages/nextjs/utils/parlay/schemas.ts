import { MAX_LEGS, MIN_LEGS, MIN_STAKE_USDC } from "./constants";
import { z } from "zod";
...
export const QuoteRequestSchema = QuoteBaseSchema.refine(data => data.legIds.length === data.outcomes.length, {
...
const LegProbBaseSchema = QuoteBaseSchema.extend({
  probabilities: z.array(z.number().int().min(1).max(999_999)).min(MIN_LEGS).max(MAX_LEGS),
});
...
export const RiskAssessRequestSchema = LegProbBaseSchema.extend({
  ...
  categories: z
    .array(
      z
        .string()
        .regex(/^[\w \-./]+$/, "Category must contain only alphanumeric, underscore, space, hyphen, dot, or slash"),
    )
    .optional(),
})
  ...
  .refine(data => !data.categories || data.categories.length === data.legIds.length, {
...
export const AgentQuoteRequestSchema = QuoteBaseSchema.extend({
  ...
  riskTolerance: z.enum(["conservative", "moderate", "aggressive"]),
}).refine(data => data.legIds.length === data.outcomes.length, {
  message: "legIds and outcomes must have the same length",
});
