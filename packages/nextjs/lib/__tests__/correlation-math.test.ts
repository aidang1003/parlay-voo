...
describe("end-to-end pricing parity", () => {
  ...
  it("reproduces the on-chain ticket multiplier formula", () => {
    ...
    const final = applyCorrelation(feeAdjusted, [], CORRELATION_ASYMPTOTE_BPS, CORRELATION_HALF_SAT_PPM);
  ...
  it("randomized 1000-vector property: fee+corr never inflates", () => {
    ...
    for (let i = 0; i < 1000; i++) {
      ...
      const finalMul = applyCorrelation(feeAdjusted, groupSizes, CORRELATION_ASYMPTOTE_BPS, CORRELATION_HALF_SAT_PPM);
