...
const STEPS: FTUEStep[] = [
  {
    ...
    title: "Place a Bet",
    description:
      "Pick 2-5 prediction legs from the markets and choose Yes or No on each. Your odds multiply together for a bigger payout.",
    position: "top",
  },
  {
...
export function FTUESpotlight() {
  ...
  return createPortal(
    <>
      ...
      {rect && (
        ...
        >
          <div className="mb-3 flex items-center justify-between">
            ...
            <button onClick={skip} className="text-xs text-gray-500 transition-colors hover:text-gray-300">
              Skip
            </button>
          </div>
            ...
            {steps.map((_, i) => (
              <div
                ...
                className={`h-1.5 w-1.5 rounded-full transition-all ${
                  i === stepIndex ? "w-4 bg-brand-pink" : i < stepIndex ? "bg-brand-pink/50" : "bg-gray-700"
                }`}
              />
            ))}
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <button onClick={next} className="btn-gradient rounded-lg px-4 py-1.5 text-xs font-bold text-white">
      ...
      {!rect && targetExists && <div className="ftue-overlay" onClick={next} />}
    </>,
    document.body,
  );
}

function getTooltipStyle(rect: SpotlightRect, position: string): React.CSSProperties {
  const gap = 16;
  switch (position) {
    case "top":
      return {
