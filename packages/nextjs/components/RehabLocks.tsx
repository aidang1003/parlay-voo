...
import { LockTier, useLockPositions } from "~~/lib/hooks";
...
export function RehabLocks() {
  ...
  const sharePrice = typeof assetsPerShare === "bigint" ? Number(formatUnits(assetsPerShare, 6)) : 1.0;
    ...
    <section className="mx-auto max-w-3xl animate-fade-in-up">
      <div className="rounded-2xl border border-amber-500/10 bg-gradient-to-br from-amber-950/20 to-gray-900/50 p-6">
        ...
        <p className="mb-5 text-sm text-gray-400">
          These positions marked the end of a losing streak — principal has already been distributed to LPs. They
          don&apos;t earn fees. To rebuild a fee-earning lock, graduate a partial position to full (or catch a lossless
          win) — see the builder for details.
        </p>
        ...
        <div className="space-y-3">
          {leastPositions.map(({ id, position }) => {
            ...
            return (
              <div key={id.toString()} className="rounded-xl border border-white/5 bg-white/[0.02] p-4">
                <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
                  <div className="min-w-[120px]">
                    ...
                    <p className="text-sm font-bold tabular-nums text-white">
                      {principalFloat.toFixed(2)} VOO <span className="text-gray-500">(${valueAtBurn})</span>
                    </p>
                  </div>
                  <div>
                    <p className="text-[10px] font-medium uppercase tracking-wider text-gray-500">Status</p>
                    <p className="text-sm font-semibold text-gray-400">Principal routed to LPs</p>
                  </div>
                  <div className="ml-auto text-right">
                    <p className="text-[10px] font-medium uppercase tracking-wider text-gray-500">Created</p>
