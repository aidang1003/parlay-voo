        ...
        <div>
          <h1 className="text-3xl font-black text-white">
            ...
            {viewTab === "myTickets" && tickets.length > 0 && (
              <span className="text-lg font-medium text-gray-500"> ({tickets.length})</span>
            )}
          </h1>
            ...
            <div className="flex gap-1 rounded-xl border border-white/5 bg-gray-900/50 p-1">
              {TABS.map(tab => (
                <button
                  ...
                  className={`flex-1 rounded-lg px-3 py-2 text-sm font-semibold transition-all ${
                    activeTab === tab.key ? "gradient-bg text-white shadow-lg" : "text-gray-500 hover:text-gray-300"
                  }`}
                >
                  {tab.label}
                  {tabCounts[tab.key] > 0 && (
                    <span
                      className={`ml-1.5 inline-flex h-5 min-w-[20px] items-center justify-center rounded-full px-1.5 text-[10px] font-bold ${
                        activeTab === tab.key ? "bg-white/20 text-white" : "bg-white/5 text-gray-600"
                      }`}
                    >
                      {tabCounts[tab.key]}
                    </span>
