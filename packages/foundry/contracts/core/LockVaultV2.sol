...
contract LockVaultV2 is Ownable, ReentrancyGuard, ILockVault {
    ...
    function rehabLock(address user, uint256 shares, uint256 duration, ILockVault.Tier tier)
        external
        override
        onlyVault
        nonReentrant
    {
        ...
        require(tier == ILockVault.Tier.PARTIAL || tier == ILockVault.Tier.LEAST, "LockVaultV2: invalid rehab tier");

        voo.safeTransferFrom(msg.sender, address(this), shares);

        totalLockedShares += shares;

        uint256 unlockAt = tier == ILockVault.Tier.PARTIAL ? type(uint256).max : block.timestamp + duration;
