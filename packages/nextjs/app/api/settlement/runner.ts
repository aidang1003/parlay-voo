...
import { type MarketRow, getUnresolvedPolymarketLegs, recordResolution } from "~~/lib/db/client";
import { PolymarketClient } from "~~/lib/polymarket/client";
import { type ClaimOutcome, encodeClaim } from "~~/lib/uma/claim";
import {
  ANVIL_ACCOUNT_0_KEY,
...
const ORACLE_READ_ABI = parseAbi(["function canResolve(uint256 legId) view returns (bool)"]);
...
function resolveChainSetup() {
  ...
  const pk = (process.env.WARM_DEPLOYER_PRIVATE_KEY ??
    (chainId === LOCAL_CHAIN_ID ? ANVIL_ACCOUNT_0_KEY : undefined)) as Hex | undefined;
  if (!pk) {
    throw new Error(
      "WARM_DEPLOYER_PRIVATE_KEY not set (required on non-local chains to sign AdminOracleAdapter.resolve + settleTicket)",
    );
  }
