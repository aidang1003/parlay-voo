    ...
    <div className="space-y-16">
      ...
      <section className="relative pt-4 text-center">
        ...
        <p className="mx-auto mt-4 max-w-lg text-sm text-gray-400">
          Real-time performance metrics for ParlayVoo&apos;s autonomous on-chain agents. No login required.
        </p>
      </section>
        ...
        <div className="glass-card p-6">
          <div className="mb-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">Discovers NBA games, registers legs, resolves outcomes</p>
            </div>
          </div>
          <div className="space-y-3">
            <div className="flex items-center justify-between rounded-lg bg-white/5 px-3 py-2">
              <span className="text-xs text-gray-400">Markets Registered</span>
              <span className="text-sm font-bold text-white">{stats?.legCount ?? "--"}</span>
            </div>
            <div className="flex items-center justify-between rounded-lg bg-white/5 px-3 py-2">
              <span className="text-xs text-gray-400">Data Source</span>
              <span className="text-sm font-bold text-brand-pink">BallDontLie API</span>
            </div>
            <div className="flex items-center justify-between rounded-lg bg-white/5 px-3 py-2">
              <span className="text-xs text-gray-400">Resolution</span>
              <span className="text-sm font-bold text-neon-green">Automatic (game scores)</span>
            </div>
          </div>
        </div>
        ...
        <div className="glass-card p-6">
          <div className="mb-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">Permissionless ticket settlement, releases vault reserves</p>
            </div>
          </div>
          <div className="space-y-3">
            <div className="flex items-center justify-between rounded-lg bg-white/5 px-3 py-2">
              <span className="text-xs text-gray-400">Tickets in System</span>
              <span className="text-sm font-bold text-white">{stats?.ticketCount ?? "--"}</span>
            </div>
            <div className="flex items-center justify-between rounded-lg bg-white/5 px-3 py-2">
              <span className="text-xs text-gray-400">Settlement</span>
              <span className="text-sm font-bold text-neon-green">Permissionless</span>
            </div>
            <div className="flex items-center justify-between rounded-lg bg-white/5 px-3 py-2">
      ...
      <section>
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-xl font-bold text-white">Recent Agent Transactions</h2>
          <button
            onClick={() => {
              setLoading(true);
              fetchStats();
              ...
              <tbody>
                {loading && !stats ? (
                  ...
                  stats?.recentTxs.map(tx => (
                    <tr key={tx.hash} className="border-b border-white/5 transition-colors hover:bg-white/[0.02]">
                      ...
                      <td className="px-4 py-3 font-mono text-xs text-gray-400">{formatGas(tx.gasCost)}</td>
                      <td className="px-4 py-3 text-xs text-gray-500">{timeAgo(tx.timestamp)}</td>
                      <td className="px-4 py-3">
                        <span
      ...
      <section className="mx-auto max-w-2xl">
        <div className="glass-card p-8 text-center">
          ...
          <p className="mt-4 text-xs text-gray-600">
            Builder code: <code className="text-gray-400">parlayvoo</code> (ERC-8021 attribution on every tx)
          </p>
        </div>
      </section>
    </div>
...
function StatCard({
  ...
  const content = (
    <div className="glass-card flex flex-col items-center p-5 text-center transition-all hover:bg-white/[0.04]">
      ...
      <p className={`mt-1 text-lg font-bold text-white ${loading ? "animate-pulse" : ""}`}>{value}</p>
    </div>
  );
