...
USDC amounts are in 6-decimal format on-chain but display as human-readable dollars.`,
    ...
    tools: {
      ...
      get_quote: tool({
        ...
        parameters: z.object({
          legIds: z.array(z.number()).min(2).max(5).describe("Array of leg IDs (2-5 legs)"),
          stake: z.number().positive().describe("Stake amount in USDC"),
        }),
      ...
      assess_risk: tool({
        ...
        parameters: z.object({
          legIds: z.array(z.number()).min(2).max(5).describe("Array of leg IDs"),
          stake: z.number().positive().describe("Proposed stake in USDC"),
          bankroll: z.number().positive().optional().describe("Total bankroll in USDC (default 1000)"),
        }),
      ...
      get_leg_status: tool({
        description: "Get the status of a specific betting leg by ID, including on-chain registration status.",
        parameters: z.object({
