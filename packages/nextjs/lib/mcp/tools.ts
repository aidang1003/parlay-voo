...
import { type Abi, createPublicClient, formatUnits, http } from "viem";
...
import type { Leg, Market, RiskProfile } from "~~/utils/parlay";
...
// Lazy: getRpcUrl() throws if ALCHEMY_API_KEY isn't set, which happens at
// build time when env isn't loaded. Build the public client on first use.
// `any` at the module boundary because viem's createPublicClient return type
// narrows on the chain literal, which makes a memoized variable awkward.
let _client: any;
function client(): any {
  if (!_client) {
    _client = createPublicClient({ chain, transport: http(getRpcUrl(chainId)) });
  }
  return _client;
}
...
export async function getQuote(input: { legIds: number[]; stake: number }): Promise<{
  valid: boolean;
  multiplier: string;
...
export async function getVaultHealth(): Promise<{
  ...
  try {
    ...
    const utilPct = totalAssets > 0n ? `${((Number(totalReserved) / Number(totalAssets)) * 100).toFixed(2)}%` : "0.00%";
...
export async function assessRisk(input: { legIds: number[]; stake: number; bankroll?: number }): Promise<{
  action: string;
  ...
  if (winProbability < caps.minWinProb) {
    warnings.push(
      `Win probability ${(winProbability * 100).toFixed(2)}% is below moderate minimum of ${(caps.minWinProb * 100).toFixed(0)}%`,
    );
  }
