...
import { useCreditBalance, useLockPositions, useLockStats, useUSDCBalance, useVaultPosition } from "~~/lib/hooks";
      ...
      {open && (
        ...
        >
          <div className="mb-3 border-b border-white/10 pb-3">
            <p className="text-xs font-semibold uppercase tracking-wider text-gray-400">{token.label}</p>
            {token.subtitle && <p className="mt-0.5 text-[11px] text-gray-500">{token.subtitle}</p>}
            <p className="mt-1 text-base font-semibold text-brand-pink">${formatUSDC(usdcBigInt)}</p>
          </div>
          ...
          {creditBigInt > 0n && <PopoverRow label="Lossless credit" value={`$${formatUSDC(creditBigInt)}`} />}
        </div>
      )}
    </div>
  );
