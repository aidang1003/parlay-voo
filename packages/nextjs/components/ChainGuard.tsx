...
export function ChainGuard() {
  ...
  const handleSwitch = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : `Failed to switch — add ${pinnedName} to your wallet first`);
    }
  };
  ...
  return (
    <div className="border-b border-yellow-500/20 bg-yellow-500/10 text-yellow-200">
      <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-3 px-4 py-2 text-sm sm:px-6">
        <span>
          Wallet is on <b>{walletName}</b>, but this app is pinned to <b>{pinnedName}</b>. Transactions will hang until
          you switch.
