...
import { MAX_LEGS } from "~~/utils/parlay";
...
function linearGridlines(maxVal: number): number[] {
  ...
  const interval = normalized <= 2 ? step * 0.5 : normalized <= 5 ? step : step * 2;
  const lines: number[] = [];
...
export function MultiplierClimb({
  ...
  const color = riskLevel <= 0.4 ? "#22c55e" : riskLevel <= 0.7 ? "#eab308" : "#ef4444";

  const ceiling = useMemo(() => niceCeiling(Math.max(runningMultiplier, 2), isLog), [runningMultiplier, isLog]);
  ...
  return (
    <div className="relative flex flex-col items-center gap-4">
      ...
      >
        <span className="text-sm font-medium uppercase tracking-wider text-gray-500">Multiplier</span>
        <div
          className={`font-black tabular-nums transition-colors duration-300 ${
            animated ? `text-6xl ${crashed ? "text-glow-red" : "text-glow-green animate-glow-pulse"}` : "text-5xl"
          }`}
      ...
      >
        ...
        >
          <span
            className={`rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider transition-all ${
              !isLog ? "bg-brand-pink/15 text-brand-pink" : "text-gray-600"
            }`}
          >
            Lin
          </span>
          <span
            className={`rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wider transition-all ${
              isLog ? "bg-brand-purple/15 text-brand-purple-1" : "text-gray-600"
            }`}
          >
            Log
          </span>
        ...
        <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="absolute inset-0 h-full w-full">
          ...
          {pathD && (
            <>
              <defs>
                <linearGradient id="climbGrad" x1="0%" y1="0%" x2="0%" y2="100%">
