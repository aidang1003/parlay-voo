...
import { createPublicClient, formatEther, formatUnits, http, parseAbi, toFunctionSelector } from "viem";
import { baseSepolia } from "viem/chains";
import deployedContracts from "~~/contracts/deployedContracts";
import { BASE_SEPOLIA_CHAIN_ID, type SupportedChainId, getRpcUrl } from "~~/utils/parlay";

const AGENT_WALLET = (process.env.NEXT_PUBLIC_AGENT_WALLET ??
  "0x1214ACab3De95D9C72354562D223f45e16a80389") as `0x${string}`;

const STATS_CHAIN_ID = Number(process.env.NEXT_PUBLIC_CHAIN_ID ?? String(BASE_SEPOLIA_CHAIN_ID)) as SupportedChainId;
// Lazy: avoids build-time eval when ALCHEMY_API_KEY isn't present in the build env.
const getStatsRpcUrl = () => process.env.RPC_URL ?? getRpcUrl(STATS_CHAIN_ID);

const USDC_ABI = parseAbi(["function balanceOf(address) view returns (uint256)"]);
const REGISTRY_ABI = parseAbi(["function legCount() view returns (uint256)"]);
const ENGINE_ABI = parseAbi(["function ticketCount() view returns (uint256)"]);
...
export async function GET() {
  ...
  try {
    const client = createPublicClient({
      chain: baseSepolia,
      transport: http(getStatsRpcUrl()),
    });

    const chainContracts = (deployedContracts[STATS_CHAIN_ID as keyof typeof deployedContracts] ??
      Object.values(deployedContracts)[0]) as Record<string, { address: `0x${string}` }>;
    ...
    try {
      ...
      if (json.status === "1" && Array.isArray(json.result)) {
        ...
        const agentTxs = json.result.filter(tx => tx.from.toLowerCase() === AGENT_WALLET.toLowerCase());
    ...
    return NextResponse.json({ error: "Failed to fetch agent stats", detail: msg }, { status: 500 });
  }
}
