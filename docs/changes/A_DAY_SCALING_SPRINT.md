...
### F-5 Trustless oracle — UMA OOv3 on Base (replace admin backdoor) ✅ COMPLETE
- **Shipped:** see [`F5_UMA_ORACLE.md`](F5_UMA_ORACLE.md) for the full change record. `UmaOracleAdapter.sol` wraps UMA Optimistic Oracle V3 on Base; `OptimisticOracleAdapter.sol` deleted. No `onlyOwner` function in the oracle path can mutate outcome state. Live snapshot is in `docs/ARCHITECTURE.md` (Oracle Adapters) + `docs/THREAT_MODEL.md` (T2/T3) + `docs/DEPLOYMENT.md`.
- **Time:** 2-3 days
...
- ✅ COMPLETE **MockUSDC mint reverts at the 100,000 slider max.** Cap was 10_000e6 (10,000 USDC) per call; raised to `MAX_MINT = 10_000_000e6` (10M USDC) in `packages/foundry/src/MockUSDC.sol`. Slider max (100,000) is now well below the cap.
