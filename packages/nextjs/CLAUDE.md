...
- `lib/wagmi.ts` -- wagmi config (foundry + baseSepolia), RPC URLs from `@parlayvoo/shared` chains.ts
...
- Protocol constants (fees, limits) import from `@parlayvoo/shared` constants.ts -- no local config file.
- Chain config (RPC URLs, chain IDs) imports from `@parlayvoo/shared` chains.ts.
