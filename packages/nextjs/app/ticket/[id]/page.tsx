  ...
  if (!onChainTicket) {
    return (
      <div className="py-20 text-center">
        ...
        <Link href="/tickets" className="mt-4 inline-block text-sm text-brand-pink hover:underline">
          Back to My Tickets
        </Link>
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-lg space-y-6">
      ...
      <div className="text-center">
        <h1 className="text-2xl font-bold text-white">Ticket #{idStr}</h1>
      ...
      {ticket.status === "Active" && (
        <div className="space-y-3">
          <MultiplierClimb legMultipliers={legMultipliers} crashed={crashed} resolvedUpTo={resolvedWon} animated />
          {/* Live stats bar */}
      ...
      {isSettled && <SettledClimb legMultipliers={legMultipliers} crashed={crashed} resolvedWon={resolvedWon} />}
...
function SettledClimb({
  ...
  const { replaying, replayStep, replayCrashed, startReplay, stopReplay } = useReplay(resolvedWon, crashed);
